...
def get_fpr_fnr_sensitive_features(y_true, y_pred, x_control, sensitive_attrs, verbose = False):
    ...
    for s in sensitive_attrs:
        ...
        for s_val in sorted(list(set(s_attr_vals))):
            ...
            fp = np.sum(np.sum(np.array(np.logical_and(y_true_local == -1.0, y_pred_local == +1.0)))) # something which is -ve but is misclassified as +ve
            fn = np.sum(np.sum(np.array(np.logical_and(y_true_local == +1.0, y_pred_local == -1.0)))) # something which is +ve but is misclassified as -ve
            tp = np.sum(np.sum(np.array(np.logical_and(y_true_local == +1.0, y_pred_local == +1.0)))) # something which is +ve AND is correctly classified as +ve
            tn = np.sum(np.sum(np.array(np.logical_and(y_true_local == -1.0, y_pred_local == -1.0)))) # something which is -ve AND is correctly classified as -ve

            all_neg = np.sum(np.array(y_true_local == -1.0))
            all_pos = np.sum(np.array(y_true_local == +1.0))
...
def get_sensitive_attr_constraint_fpr_fnr_cov(model, x_arr, y_arr_true, y_arr_dist_boundary, x_control_arr, verbose=False):
    ...
    for ct in [0,1,2]:
        s_val_to_avg[ct][0] = s_val_to_total[ct][1] / (float(s_val_to_total[ct][0]) + float(s_val_to_total[ct][1])) # N1 / N
        s_val_to_avg[ct][1] = 1.0 - s_val_to_avg[ct][0] # N0 / N
